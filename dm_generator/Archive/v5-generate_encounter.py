...
def write_markdown(encounter, monsters):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        sources = [m.get('source', '?') for m in monsters]  # Get sources.
        f.write(f"sources: {', '.join(sources)}\n")  # Write sources to YAML
        f.write("---\n\n")

        f.write(f"# {encounter['name']}\n\n")

        for monster in monsters:
            #  Create an Obsidian link.  The assumption is that the monster file
            #  name in your vault is the same as the monster's name.
...
def main():
    while True:
        ...
        if not matching:
            all_envs = set(e for m in bestiary for e in m.get("environment", [])) # Fixed this line
        ...
        if choice == "save":
            write_markdown(encounter, selected)
            break
        elif choice == "new":
            continue
        elif choice == "exit":
